...
if [ -f "memory_dump_samples/macos_sample.mem" ]; then
    python3 -m unified_forensics experiment memory_dump_samples/macos_sample.mem --os-type macos --rates 1 --rates 10 --output analysis_results/test_experimental_macos.json
    if [ $? -ne 0 ]; then
    ...
    echo "WARNING: macOS sample not found, skipping experimental test"
fi
